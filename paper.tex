...
\begin{compactitem}
    ...
    % TODO: the point should be against gathering global similarity, as most probably people will think of a respect when they give a similarity judgement.
    \item ``[S]imilarity\ldots is a relationship between two objects \emph{under a given description}'' \cite{WCS:WCS1282}. Global similarity judgements omit the description and possibly give imprecise global similarity. Is \textit{apple}'s similarity to \textit{orange} as a company the same as a fruit? Is \textit{apple}'s and \textit{orange}'s taste as similar as their geographical spread?
